<template>
  <div id="translation-page">
    <div class="q-pa-md">
      <div class="row">
        <div class="text-h5">Hasil Translasi citra asli </div>
      </div>
...
const getImage = async () => {
  return await api.get(`image/${$state.translationImageName}`);
};
...
watchEffect(() => {
  console.log(dialogState.translateDialogChange);

  // Ensure dialogState.translateDialogChange is not null or undefined
  if (dialogState.translateDialogChange === true || dialogState.translateDialogChange == false) {
    fetchImage();
  }
});
...
const handleSave = async () => {
  ...
  link.href = blobUrl;
  link.download = 'translate_image.jpg';
  link.click();
