<template>
  <div id="binary-page">
    <div class="q-pa-md">
      <div class="row">
        <div class="text-h5">Hasil Pencerahan citra </div>
      </div>
...
const { brightenImageName, $state } = useImageStore();
const imageUrl = ref('');
...
watchEffect(async () => {
  if ($state.brightenImageName !== brightenImageName) {
    await initImage()
  }
})
...
const getImage = async () => {
  return await api.get(`image/${$state.brightenImageName}`);
